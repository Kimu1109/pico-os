...
- [x] ダイアログ系統
  - [x] キーボード日本語 (フリック入力+SKK変換。カナ/送りキーが変換中でないときカーソル移動キーになる)
  - [x] キーボード英語 (最下段に←→。幅の都合でラベルを かな / enter / abc に短縮、シフトでも表記は変えない)
  - [x] キーボード数字（電卓用） (KeyboardNum。Digit/Arith/Mathの3タブ、使えるタブを呼び出し側で制限可)
  - [x] キーボードのカーソル移動 (数字に続き日本語/英語も挿入・削除がカーソル位置基準に。位置はLabel::setCursorToByteOffset()で受け渡す)
  - [x] メッセージダイアログ
  - [x] インプットダイアログ
  - [x] ファイル保存ダイアログ
  - [x] ファイル選択ダイアログ
