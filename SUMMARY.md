...
| 9 | [GameBoyエミュ](#9-gameboyエミュ) | 🔨 Peanut-GBを採用。第1段(256KBまでのROMをRAMへ丸ごと読む)がPCで動作。実機は未確認 |
| 10 | [外部コントローラー](#10-外部コントローラー) | 🔨 入力の窓口(`PadFunctions`)とUSBシリアル経由のPCキーボード入力、GBエミュ・Luaへの組み込みまで。実物(Wiiクラシックコントローラー、I2C)は未 |
| 11 | [Chiptuneを再生](#11-chiptuneを再生) | 🔨 4チャンネルの音源と曲(MML)が2コア目で鳴り、Luaとミュージックアプリから使える(PCで確認)。MIDIはPCの`midi2mml.py`で取り込める。GB対応は未。実機は未確認 |
...
- [x] 配線/方式を考えよう
- [ ] 入力を受け取ろう
  - [x] USBシリアル経由でPCのキーボードから
  - [ ] Wiiクラシックコントローラー(I2C)から
- [x] 実際にアプリに組み込めるようにしよう

### 11. [Chiptuneを再生](#8-11-未着手の大項目)

- [x] とりあえず再生
  - [x] 配線/方式を決める(I2S + MAX98357A)
  - [x] 出力の土台(アンプの検出・未接続時の扱い)
  - [x] 音源(矩形波/三角波/ノイズの合成)
  - [x] 2コア目での合成
- [ ] GB対応
- [x] 標準ファイル形式を探す/考える
  - [x] 形式を選ぶ(MML)
  - [x] 仕様を固める(MUSIC_FORMAT.md)
  - [x] 読み取りと2コア目のシーケンサー
  - [x] ミュージックアプリ
  - [x] MIDIからの変換
- [ ] アプリ対応
...
いずれも**土台から設計する前提**(9と11は着手済み)。既存機能の拡張ではないので、相談は設計の段から始まる。
