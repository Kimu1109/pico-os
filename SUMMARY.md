...
| 7 | [標準アプリ開発](#7-標準アプリ開発) | 🔨 Markdownブラウザ / 時計が完了。他は未着手 |
...
| 電卓 | ⬜ 部品: `KeyboardNum` / `NumberInput`。**最小コストで1本書ける** |
| ファイルエクスプローラー | ⬜ `FileExplorer`ウィジェットは実装済み。アプリ(Scene + `App_List`への登録)が無いだけ |
