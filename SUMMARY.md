...
- 結果一覧(`ScrollList`)をタップすると、その場で下の詳細欄(`Label`)へ表示用語句と
  説明/訳の全文を出す。一覧・詳細を別シーン/別ダイアログへ分けなかったのは、
  辞書を引く操作は「一覧を見ながら次を引く」往復が多く、都度画面遷移すると
  かえって使いにくいため。
- **このシーンのオブジェクトはMarkdownSceneと同じく「数十バイト」の他シーンより大きい**
  (`WordDictionary`が実測約43KB)。辞書アプリは自分の上へ別シーンをPushしないので、
  シーンスタックに積まれたまま残り続ける実害は無い。
- 詳細欄はLabelの`maxHeight`で高さを決め打ちしており、収まらない分は切り詰まる
  (スクロールはしない)。実データで説明が512Bを超えるのは0.3%程度なので大半は
  収まるが、長文の説明を最後まで読ませたくなったら`ScrollContainer`(現状未使用の
  汎用部品)への置き換えが候補になる。
